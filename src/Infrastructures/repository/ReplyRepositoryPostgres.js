...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async verifyReplyAvailability(replyId, commentId) {
    ...
    if (!result.rowCount) {
      throw new NotFoundError('reply tidak dapat ditemukan di database');
    }

    if (result.rows[0].is_delete) {
      throw new NotFoundError('data reply tidak valid');
    }

    if (result.rows[0].comment !== commentId) {
      throw new NotFoundError('reply dalam comment tidak dapat ditemukan di database');
    }
  }
  ...
  async createReply(userId, commentId, newReply) {
